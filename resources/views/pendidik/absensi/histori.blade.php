...
@section('title', 'Histori Mengajar - Cakra Krisna Manggala')
            ...
            <div class="col-sm-4 col-md-3">
                ...
                <select id="kelas" name="kelas" class="form-select">
                    @foreach ($kelas as $item)
                        <option value="{{ $item->id }}" @selected((int) $kelas_id === (int) $item->id)>{{ $item->nama }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-sm-4 col-md-3">
        ...
        @forelse ($jadwal as $item)
            <div class="col-md-6 col-xl-3">
                <section class="ck-card p-4 h-100">
                    ...
                    <p class="mb-1">Pulang <b>{{ \Carbon\Carbon::parse($item->pulang)->isoFormat('HH:mm') }}</b></p>
                    <p class="mb-0 ck-hint">Jurnal<br>"{{ $item->jurnal }}"</p>
                </section>
            </div>
        @empty
            <div class="col-12">
                <section class="ck-card p-4">
                    <p class="ck-hint mb-0">Tidak ada histori pada filter ini.</p>
                </section>
            </div>
        @endforelse
    </div>
